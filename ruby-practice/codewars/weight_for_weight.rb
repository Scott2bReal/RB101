...
string ordered by "weights" of these numbers?  Example:
...
Output: String with numbers ordered by "weight" as described

Data Structure: Array, hash

Algorithm:
  - group numbers by weight into hash
  - sort that hash by key (weight)
  - sort each value array (as string)
...
def order_weight(string)
  nums = string.split(' ')
  weights = nums.group_by { |num| num.to_i.digits.sum }
  sorted_weights = weights.sort.to_h.values.each { |arr| arr.sort! }
  sorted_weights.flatten.join(' ')
end
