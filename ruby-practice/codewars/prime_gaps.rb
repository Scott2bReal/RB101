...
gap of g between the limits m, n if these numbers exist otherwise `nil or null
...
Algorithm:
  Require 'prime'
  Init array primes_in_range = (m..n)
  Select only prime numbers from this array
  Iterate thru primes_in_range with index
    if element at idx + 1 - element == gap, return array with those elements
    if idx == primes_in_range.size return nil
...
def prime_gap(gap, start, finish)
  primes_in_range = (start..finish).select(&:prime?)
  primes_in_range.each_with_index do |num1, idx|
    return nil if idx == primes_in_range.size - 1
    num2 = primes_in_range[idx + 1]
